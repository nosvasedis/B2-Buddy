...
import { getDailyWord, CoachingBrief } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const loadDailyWord = async (currentStats: UserStats) => {
    ...
    try {
        const text = await getDailyWord();
        const lines = text.split('\n');
        const w: DailyWord = {
            word: lines.find(l => l.startsWith('Word:'))?.replace('Word:', '').replace(/\*\*/g, '').trim() || 'Resilience',
            type: lines.find(l => l.startsWith('Type:'))?.replace('Type:', '').trim() || 'Noun',
            definition: lines.find(l => l.startsWith('Meaning:'))?.replace('Meaning:', '').trim() || 'Capacity to recover.',
            example: lines.find(l => l.startsWith('Example:'))?.replace('Example:', '').trim() || 'She showed resilience.',
            date: today
        };
        setDailyWord(w);
  ...
  const handleSaveLesson = (topicId: string, content: LessonContent) => {
      setUserStats(prev => {
