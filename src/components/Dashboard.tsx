...
import { generateCoachingBrief, CoachingBrief } from '../services/openRouterService';
...
const FALLBACK_BRIEF: CoachingBrief = {
    greeting: "Hey!",
    mood: "neutral",
    insight: "Ready when you are. Pick a quest and let's go!",
    mission: "Choose an activity from the hub below."
};
...
const Dashboard: React.FC<DashboardProps> = ({ onChangeSection, dailyWord, stats, coachBrief, setCoachBrief, onCollectDailyWord }) => {
    ...
    const briefRequestInFlightRef = useRef(false);

    useEffect(() => {
        if (!coachBrief && !briefRequestInFlightRef.current && stats?.userId) {
            briefRequestInFlightRef.current = true;
            generateCoachingBrief(stats)
                .then((brief) => {
                    setCoachBrief(brief);
                })
                .catch(() => {
                    setCoachBrief(FALLBACK_BRIEF);
                })
                .finally(() => {
                    briefRequestInFlightRef.current = false;
                });
        }
    ...
    }, [coachBrief, stats, setCoachBrief]);
    ...
    return (
        <div className="max-w-7xl mx-auto pb-20 pt-4 px-2 md:px-4 animate-app-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 auto-rows-[minmax(180px,auto)] mb-6">
                ...
                <BentoItem onClick={onChangeSection} section={AppSection.EXAM} colorTheme="amber" icon="🏆" title="Exam Mode" subtitle="ECCE Mock 100" className="col-span-1" delay="0.2s" />
                <StrategyCorner />
                ...
                <BentoItem onClick={onChangeSection} section={AppSection.ESSAY_TUTOR} colorTheme="pink" icon="📝" title="Writing" subtitle="Ink Station + AI" className="col-span-1" delay="0.4s" />
            </div>
        </div>
    );
};
