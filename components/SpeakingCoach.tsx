...
const SpeakingCoach: React.FC<SpeakingCoachProps> = ({ onSessionComplete, onUpdateStats, stats }) => {
    ...
    const startTest = async () => {
        ...
        try {
            ...
            const instruction = `You are an official Cambridge Assessment English Examiner. 
            Part of the test: B2 First (FCE) ${activePart}.
            CANDIDATE: 13-year-old student.
            STRICT RULES:
            - Stick exactly to the official script phrasing.
            - Part 1: Ask 2-3 personal questions.
            - Part 2: Tell them to compare the two photos and answer the question for 1 minute.
            - Part 3: Ask them to discuss the central question with you as a partner.
            - Part 4: Ask follow-up questions about the theme.
            - Stop immediately when time is up.`;

            /* Fix: Utilizing connectLiveSession which returns a session promise to strictly follow guidelines */
            const sessionPromise = connectLiveSession(
                async (msg: LiveServerMessage) => {
                    /* Fix: Added interruption handling logic to stop playback when model is interrupted */
                    ...
                    if (msg.serverContent?.modelTurn?.parts?.[0]?.inlineData) {
                        ...
                        source.onended = () => {
                            ...
                            setIsExaminerSpeaking(activeSourcesRef.current.size > 0);
                        };
                    }
                    if (msg.serverContent?.outputTranscription?.text) {
                        transcriptRef.current += `\nExaminer: ${msg.serverContent.outputTranscription.text}`;
                        setTranscript(transcriptRef.current);
                    }
                    if (msg.serverContent?.inputTranscription?.text) {
                        transcriptRef.current += `\nCandidate: ${msg.serverContent.inputTranscription.text}`;
                        setTranscript(transcriptRef.current);
                    }
                },
                () => console.log("Open"),
            ...
            processor.onaudioprocess = (e) => {
                ...
                const pcmBlob = createPCMBlob(ds, 16000);
    ...
    useEffect(() => {
        let interval: any;
        if (state === 'EXAM_LIVE' && timeLeft > 0) {
            interval = setInterval(() => setTimeLeft(prev => prev - 1), 1000);
        } else if (state === 'EXAM_LIVE' && timeLeft === 0) {
            finishExam();
        }
