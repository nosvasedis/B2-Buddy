...
import { lookupWord, DictionaryResult, generateReviewQuiz, checkExerciseAnswer } from '../services/geminiService';
...
const DictionaryVault: React.FC<DictionaryVaultProps> = ({ stats, onUpdateStats, onAddVocabItem, onDeleteVocabItem, initialSearchTerm, onClearInitialSearch }) => {
    ...
    useEffect(() => {
        if (initialSearchTerm) {
            ...
            const performAutoSequence = async () => {
                ...
                // We just look it up.
                const res = await lookupWord(initialSearchTerm);
                if (res) {
                    setResult(res);
    ...
    const handleSearch = async (e?: React.FormEvent) => {
        ...
        if (existing) setCollected(true);
        const res = await lookupWord(searchTerm);
