...
import { generateListeningExercise, generateMultiSpeakerSpeech, generateSpeech, decodeAudio, pcmToWav } from '../services/geminiService';
...
const SmartAudioPlayer = ({ 
    ...
    return (
        <div className="fixed bottom-0 left-0 w-full z-[100] px-2 md:px-4 pb-4 pt-2 bg-gradient-to-t from-white via-white/90 to-transparent dark:from-slate-950 dark:via-slate-950/90 pointer-events-none transition-all duration-300">
            ...
            <audio ref={audioRef} src={src || undefined} preload="auto" onError={(e) => console.error("Audio Error", e)} />
        </div>
    );
};
...
const ListeningLounge: React.FC<ListeningLoungeProps> = ({ stats, onUpdateStats }) => {
    ...
    const prepareAudio = async (ex: ListeningExercise, mode: ListeningType, sessionId: string) => {
        if (mode === 'PART1_MULTIPLE_CHOICE' || mode === 'PART3_MATCHING') {
            ...
            generateStepAudio(0, ex, sessionId); 
        } else {
    ...
    const generateFullAudio = async (ex: ListeningExercise, sessionId: string) => {
        const safeId = getSafeKey(ex.weekId || ex.topic);
        const cacheKey = `full_${safeId}`;
        if (sessionAudioCache.has(cacheKey)) {
            setAudioUrl(sessionAudioCache.get(cacheKey)!);
            setGenerationStep('ready');
            return;
        }
        try {
            const savedBlob = await AudioStorage.getAudio(cacheKey);
            if (savedBlob) {
                const url = URL.createObjectURL(savedBlob);
                sessionAudioCache.set(cacheKey, url);
                setAudioUrl(url);
                setGenerationStep('ready');
                return;
            }
        ...
        try {
            let base64 = "";
            if (ex.type === 'PART2_SENTENCES') {
                 const voice = getVoiceForText(ex.script);
                 base64 = await generateSpeech(ex.script, voice);
            } else {
                 base64 = await generateMultiSpeakerSpeech(ex.script);
            }
            if (generationSessionRef.current !== sessionId) return;
            const pcmBytes = decodeAudio(base64);
            const wavBlob = pcmToWav(pcmBytes); 
            AudioStorage.saveAudio(cacheKey, wavBlob);
            ...
            console.error("Full audio gen failed", e);
            setGenerationStep('error');
        }
    };

    const generateStepAudio = async (index: number, ex: ListeningExercise, sessionId: string) => {
        const safeId = getSafeKey(ex.weekId || ex.topic);
        const cacheKey = `step_${safeId}_${index}`;
        
        if (sessionAudioCache.has(cacheKey)) {
            setStepAudioUrls(prev => ({ ...prev, [index]: sessionAudioCache.get(cacheKey)! }));
            return;
        }
        ...
        try {
            ...
            if (savedBlob) {
                const url = URL.createObjectURL(savedBlob);
                sessionAudioCache.set(cacheKey, url);
                setStepAudioUrls(prev => ({ ...prev, [index]: url }));
                return;
            }

            const text = ex.audioScriptParts[index];
            let base64 = "";

            if (text.includes('Speaker A:') || text.includes('Speaker B:') || text.includes('Woman:') || text.includes('Man:')) {
                base64 = await generateMultiSpeakerSpeech(text);
            } else {
                const selectedVoice = getVoiceForText(text);
                base64 = await generateSpeech(text, selectedVoice);
            }
            ...
            console.warn("Step audio failed");
        }
    };
    ...
    const renderPart1 = () => {
        ...
        return (
            <div className="flex flex-col items-center w-full max-w-3xl mx-auto pb-48">
                ...
                />
                <SmartAudioPlayer src={currentAudio} isPlaying={isPlaying} onPlayPause={() => setIsPlaying(!isPlaying)} />
            </div>
        );
    };
        ...
        return (
            <div className="w-full max-w-4xl mx-auto pb-48">
                ...
                <TranscriptModal 
                    ...
                    userAnswers={userAnswers}
                />
                <SmartAudioPlayer src={audioUrl} isPlaying={isPlaying} onPlayPause={() => setIsPlaying(!isPlaying)} />
            </div>
        );
    };
